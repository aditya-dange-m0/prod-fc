"""
EditTools v1 - Enhanced E2B Sandbox Code Editing Operations
===========================================================

Intelligent code editing for E2B sandbox environments with multi-tenant isolation.
Supports exact, flexible, and fuzzy matching strategies for robust code modifications.

Features:
- Multi-tenant isolation (user_id + project_id)
- Exact and flexible text replacement
- Smart fuzzy matching with auto-correction
...
def _calculate_exact_replacement(
    ...
    if occurrences > 0:
        result = _safe_literal_replace(normalized_content, normalized_old, normalized_new)
...
# =============================================================================
# MAIN EDIT TOOLS CLASS
# =============================================================================

class EditTools(Toolkit):
    """
    Enhanced E2B sandbox code editing toolkit with multi-tenant isolation.
    
    Provides intelligent code editing capabilities with exact, flexible,
    and fuzzy matching strategies. Each (user_id, project_id) gets isolated
    sandbox environment for secure code modifications.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize EditTools with user and project context.
    ...
    async def edit_file(
        ...
        Examples:
            - edit_file("backend/app.py", "old_code", "new_code")
            - edit_file("backend/new.py", "", "print('hello')")  # Create file
            - edit_file("backend/utils.py", "def old()", "def new()", expected_replacements=2)
        """
        try:
            sandbox = await get_user_sandbox(self.user_id, self.project_id)
            path = validate_sandbox_path(file_path)
            
            self.logger.debug(f"Editing file: {path}")
...
📄 Content: {len(new_string)} characters, {len(new_string.splitlines())} lines"""
            ...
            if occurrences == 0:
                searched_preview = old_string[:100] + ('...' if len(old_string) > 100 else '')
                return _create_structured_error(
            ...
            try:
                ...
                self.logger.info(
                    f"Successfully edited {path} ({occurrences} replacements)"
                )
...
📈 Stats: {len(final_content)} characters, {len(final_content.splitlines())} lines"""
            ...
            self.logger.error(f"Unexpected error in edit_file: {e}")
    ...
    async def smart_edit_file(
        ...
        Examples:
            - smart_edit_file("app.py", "old_code", "new_code", "Fix login bug")
            - smart_edit_file("utils.py", "def old()", "def new()", "Rename function")
        """
        try:
            sandbox = await get_user_sandbox(self.user_id, self.project_id)
            path = validate_sandbox_path(file_path)
            ...
            self.logger.debug(f"Smart editing file: {path} - {instruction}")
...
📄 Content: {len(new_string)} characters, {len(new_string.splitlines())} lines"""
            ...
            strategies = [
                ("exact", lambda: _calculate_exact_replacement(
                    normalized_content, old_string, new_string
                )),
                ("flexible", lambda: _calculate_flexible_replacement(
                    normalized_content, old_string, new_string
                )),
                ("fuzzy", lambda: _calculate_fuzzy_replacement(
                    normalized_content, old_string, new_string
                ))
            ]
...
if __name__ == "__main__":
    print("EditTools v1 - Enhanced E2B Sandbox Code Editing Operations")
    print("Available classes: EditTools, EditOperationResult")
